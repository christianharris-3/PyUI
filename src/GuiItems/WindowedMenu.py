...
class WindowedMenu(GuiItem):
    ...
    def draw(self, screen):
        if self.enabled:
            ...
            if self.backingdraw: Draw.rect(screen, self.col,
                                           Utils.roundrect(self.x * self.dirscale[0], self.y * self.dirscale[1],
                                                           self.width * self.scale, self.height * self.scale),
                                           self.border, border_radius=int(self.roundedcorners * self.scale))
