...
class TABLE:
    ...
    def gentext(self,ui):
        ...
        for i,b in enumerate(self.labeledtitles):
            if b[0] == 'text':
                img = ui.maketext(b[1],self.titletextsize,self.titletextcol,self.titlefont,self.titlebold)
                rec = img.get_rect()
        ...
        for a in range(len(self.labeleddata)):
            ...
            for i,b in enumerate(self.labeleddata[a]):
                if b[0] == 'text':
                    img = ui.maketext(b[1],self.boxtextsize,self.boxtextcol,self.boxfont,self.boxbold)
                    rec = img.get_rect()
