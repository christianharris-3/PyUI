...
##    def makescroller(self,x,y,height,command=emptyfunction,width=15,minh=0,maxh=-1,pageh=100,starth=0,menu='main',menuexceptions=[],edgebound=(1,0,0,1),col='default',scrollercol=-1,hovercol=-1,clickcol=-1,scrollerwidth=11,runcommandat=1,clicktype=0,layer=1,ID='default',returnobj=False):
    def makescroller(self,x,y,height,command=emptyfunction,width=15,minp=0,maxp=100,pageheight=15,menu='main',ID='scroller',layer=1,roundedcorners=0,menuexceptions=[],
                 ...
                 border=3,upperborder=-1,lowerborder=-1,rightborder=-1,leftborder=-1,scalesize=True,scalex=True,scaley=True,
                 runcommandat=0,col=-1,textcol=-1,backingcol=-1,hovercol=-1,clickdownsize=4,clicktype=0,textoffsetx=0,textoffsety=0,maxwidth=-1,
                 dragable=True,colorkey=(255,255,255),spacing=-1,verticalspacing=0,horizontalspacing=8,
                 backingdraw=True,borderdraw=True,scrollercol=-1,scrollerwidth=-1,
                 slidercol=-1,sliderbordercol=-1,slidersize=-1,increment=0,sliderroundedcorners=-1,startp=0):
...
##            ID = 'scroller '
        if maxp == -1:
            maxp = height
        obj = SCROLLER(self,x,y,width,height,menu='main',ID='',layer=1,roundedcorners=0,menuexceptions=[],
                 ...
                 slidersize=slidersize,increment=increment,sliderroundedcorners=-1,minp=0,maxp=100,startp=0)
        return obj
...
##    def makeslider(self,x,y,width,height,maxp=100,menu='main',command=emptyfunction,menuexceptions=[],edgebound=(1,0,0,1),col='default',slidercol=-1,sliderbordercol=-1,hovercol=-1,clickcol=-1,clickdownsize=2,bordercol=-1,border=2,slidersize=-1,increment=0,img='none',colorkey=(255,255,255),minp=0,startp=0,style='square',roundedcorners=0,barroundedcorners=-1,dragable=True,runcommandat=1,clicktype=0,layer=1,ID='default',returnobj=False):
    def makeslider(self,x,y,width,height,menu='main',ID='slider',layer=1,roundedcorners=0,menuexceptions=[],
                 anchor=(0,0),objanchor=(0,0),center=False,centery=-1,
                 border=3,upperborder=-1,lowerborder=-1,rightborder=-1,leftborder=-1,scalesize=True,scalex=True,scaley=True,
                 command=emptyfunction,runcommandat=0,col=-1,backingcol=-1,button='default',
                 dragable=True,colorkey=(255,255,255),backingdraw=True,borderdraw=True,
                 slidersize=-1,increment=0,sliderroundedcorners=-1,minp=0,maxp=100,startp=0,direction='horizontal'):
...
##        if barroundedcorners == -1: barroundedcorners = roundedcorners
        obj = SLIDER(self,x,y,width,height,menu='main',ID='',layer=1,roundedcorners=0,menuexceptions=[],
                 anchor=(0,0),objanchor=(0,0),center=False,centery=-1,
                 border=border,upperborder=upperborder,lowerborder=lowerborder,rightborder=rightborder,leftborder=leftborder,scalesize=scalesize,scalex=scalex,scaley=scaley,
                 command=command,runcommandat=runcommandat,col=col,backingcol=backingcol,
                 dragable=dragable,colorkey=colorkey,backingdraw=backingdraw,borderdraw=borderdraw,
                 slidersize=slidersize,increment=increment,sliderroundedcorners=sliderroundedcorners,minp=minp,maxp=maxp,startp=startp,direction=direction,data=button)

##            x,y,width,height,menu,menuexceptions,edgebound,command,col,bordercol,slidercol,sliderbordercol,hovercol,clickdownsize,border,slidersize,increment,roundedcorners,barroundedcorners,img,colorkey,minp,maxp,startp,dragable,runcommandat,clicktype,layer,ID,self)
        return obj
...
class GUI_ITEM:
    ...
    def gentext(self,ui):
        if type(self.img) == str:
            ...
            self.textimage = pygame.transform.scale(self.img,(self.textsize*self.scale,self.img.get_width()*self.textsize/self.img.get_height()*self.scale))
...
class TEXTBOX(GUI_ITEM):
    ...
    def resetscroller(self,ui):
        ...
        self.scroller = ui.makescroller(self.x+self.width-15-self.rightborder/2,self.y+self.upperborder,self.height-self.upperborder-self.lowerborder,emptyfunction,15,0,self.height-self.upperborder-self.lowerborder,self.height,menu=self.menu)
    ...
    def refresh(self,ui):
        ...
        self.scroller.maxp = self.textimage.get_height()/self.scale+self.border*2
...
##        self.refresh()
    ...
    def render(self,screen,scale,mpos,mprs,mouseheld,buttondowntimer):
        if self.holding:
            ...
            self.scroll = (mpos[1]-self.holdingcords[1]-self.y)/(self.scheight/(self.maxp-self.minp))
            self.limitpos()
...
##        self.refresh(ui)
    ...
    def resetbutton(self,ui):
        ...
        else: self.button = ui.makebutton(self.x,self.y,self.text,self.textsize,self.command,self.menu,self.ID+'button',self.layer+0.01,self.roundedcorners,self.menuexceptions,width=self.slidersize,height=self.slidersize,img=self.img,dragable=self.dragable,clickdownsize=int(self.slidersize/15))
    ...
    def refreshbutton(self,ui):
        ...
        if self.direction == 'vertical': self.slidercenter = (self.x+self.width/2,self.y+self.upperborder+(self.height-self.upperborder-self.lowerborder)*((self.slider-self.minp)/(self.maxp-self.minp)))
        self.button.x = self.slidercenter[0]-self.button.width/2
        self.button.y = self.slidercenter[1]-self.button.height/2
...
##            self.prevholding = self.holding
##        else:
        self.draw(screen,self.scale,self.slidercol)
        self.limitpos(ui)
    ...
    def movetomouse(self,mpos):
        ...
        self.limitpos()
    def limitpos(self,ui):
        self.slider = (ui.mpos[0]-self.x-self.holdingcords[0])/((self.width-self.border*2)/(self.maxp-self.minp))+self.minp
        if self.increment!=0: self.slider = round(self.slider/self.increment)*self.increment
        
        if self.direction == 'horizontal':
            self.button.y = self.y+self.height/2-self.button.height/2
            if self.button.x<self.x-self.button.width/2:
                self.button.x = self.x-self.button.width/2
            elif self.button.x>self.x+self.width-self.button.width/2:
                self.button.x = self.x+self.width-self.button.width/2
...
##        self.innerrect = pygame.Rect(self.slidercenter[0]-self.slidersize/2+self.border+self.innershrunk,self.slidercenter[1]-self.slidersize/2+self.border+self.innershrunk,self.slidersize-self.border*2-self.innershrunk*2,self.slidersize-self.border*2-self.innershrunk*2)
        pygame.draw.rect(screen,self.bordercol,rectscaler(pygame.Rect(self.x,self.y,self.width,self.height),scale),border_radius=int(self.roundedcorners*scale))
        pygame.draw.rect(screen,self.col,rectscaler(pygame.Rect(self.x+self.border,self.y+self.border,(self.width-self.border*2)*((self.slider-self.minp)/(self.maxp-self.minp)),self.height-self.border*2),scale),border_radius=int(self.roundedcorners*scale))
##        if self.dragable:
##            if self.sliderimage != 0:
##                self.sliderimagerect.center = self.slidercenter
##                screen.blit(self.sliderimage,self.sliderimagerect)
##            else:
##                pygame.draw.rect(screen,self.bordercol,rectscaler(pygame.Rect(self.slidercenter[0]-self.slidersize/2,self.slidercenter[1]-self.slidersize/2,self.slidersize,self.slidersize),scale),border_radius=int(self.roundedcorners*scale))
##                pygame.draw.rect(screen,self.col,rectscaler(self.innerrect,scale),border_radius=int(self.roundedcorners*scale))
