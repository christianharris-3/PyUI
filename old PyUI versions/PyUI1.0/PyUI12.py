...
class UI:
    ...
    def rendergui(self,screen):
        items = self.buttons+self.textboxes+self.tables+self.texts+self.scrollers
    ...
    def maketext(self,x,y,text,size,menu='main',col='default',center=True,font='default',bold=False,maxwidth=-1,border=4,backingcol='default',backingdraw=0,backingwidth=-1,backingheight=-1,img='none',roundedcorners=0,layer=1,ID='default',antialiasing=True,returnobj=False):
        if font == 'default': font = self.defaultfont
        if col == 'default': col = self.defaulttextcol
        ...
        if ID == 'default':
            ID = 'text '+text
        obj = TEXT(x,y,text,size,menu,col,center,font,bold,maxwidth,border,backingcol,backingdraw,backingwidth,backingheight,img,roundedcorners,antialiasing,layer,ID,self)
        if returnobj:
            return obj
        else:
...
class TEXTBOX:
    ...
    def resetscroller(self,ui):
        ...
        self.scroller = ui.makescroller(self.x+self.width/2-15-self.border/2,self.y+self.titleheight/2+self.border,self.height-self.titleheight-self.border*2,15,0,self.height-self.titleheight-self.border,self.height-self.titleheight,returnobj=True)
    ...
    def refresh(self,ui):
        ...
        for a in temp:
            ...
            for b in a:
                ...
                elif type(b) == TEXT: self.labeleddata[-1].append(['presizedimage',b.textimage])
