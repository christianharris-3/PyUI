...
class UI:
    ...
    def rendergui(self,screen):
        ...
        items.sort(key=lambda x: x.layer,reverse=False)
        windowedmenubackings = [a[1] for a in self.windowedmenus]
        self.breakrenderloop = False
        for i,a in enumerate(items):
            if (a.menu in windowedmenubackings or (a.menu == 'universal' and not(self.activemenu in a.menuexceptions))) and self.activemenu in self.windowedmenunames:
        ...
        if self.activemenu in self.windowedmenunames:
            ...
            for i,a in enumerate(items):
                if (a.menu == self.activemenu or (a.menu == 'universal' and not(self.activemenu in a.menuexceptions))):
    ...
    def rendershape(self,name,size,col='default'):
        ...
        if name[:4] == 'tick':
            ...
            surf = self.rendershapebezier(name,size,col,backcol)
        if 'left' in name:
    ...
    def rendershaperect(self,name,size,col,backcol):
        ...
        return surf
    def rendershapebezier(self,name,size,col,backcol):
    ...
    def makebutton(self,x,y,text,textsize,command=emptyfunction,menu='main',menuexceptions=[],col='default',bordercol=-1,hovercol=-1,textcol='default',img='none',colorkey=(255,255,255),runcommandat=0,width=-1,height=-1,border=3,clickdownsize=3,verticalspacing=0,horizontalspacing=8,roundedcorners=0,clicktype=0,textoffsetx=0,textoffsety=0,toggle=True,togglecol=-1,togglehovercol=-1,toggleable=False,drawifoff=True,bindtoggle=[],dragable=False,center=True,font='default',bold=False,layer=1,ID='default',returnobj=False):
        ...
        if type(img) == str:
            if img == 'none': textimage = self.rendertextlined(text,textsize,font=font,width=width,bold=bold,center=True)#self.rendertext(text,textsize,textcol,font,bold)
    ...
    def maketext(self,x,y,text,size,menu='main',menuexceptions=[],col='default',center=True,font='default',bold=False,maxwidth=-1,border=4,backingcol='default',backingdraw=0,backingwidth=-1,backingheight=-1,img='none',colorkey=(255,255,255),roundedcorners=0,layer=1,ID='default',antialiasing=True,returnobj=False):
        if font == 'default': font = self.defaultfont
        if col == 'default': col = self.defaulttextcol
        ...
        if ID == 'default':
            ID = 'text '+text
        obj = TEXT(x,y,text,size,menu,menuexceptions,col,center,font,bold,maxwidth,border,backingcol,backingdraw,backingwidth,backingheight,img,colorkey,roundedcorners,antialiasing,layer,ID,self)
        if returnobj:
            return obj
        else:
    ...
    def animate(self):
        ...
        for a in delete:
            self.delete(a)
    def makeanimation(self,animateID,startpos,endpos,movetype='linear',length=30,acceleration=1,command=emptyfunction,runcommandat=-1,queued=True,menu=False,relativemove=False,ID='default'):
        if menu:
            items = self.buttons+self.textboxes+self.tables+self.texts+self.scrollers+self.sliders
            for a in items:
                if a.menu == animateID:
                    self.makeanimation(a.ID,startpos,endpos,movetype,length,acceleration,command,runcommandat,queued,False,relativemove)
                    runcommandat = -1
        else:
            if ID == 'default':
            ...
            if not queued:
                ...
                tofinish.sort(key=lambda x: x[1],reverse=False)
                for a in tofinish:
                    self.IDs[a[0]].finish(self)
                    self.delete(a[0])
            else:
                for a in self.animations:
                    if a.animateID == animateID:
                        wait = max([a.wait+a.length,wait])
            obj = ANIMATION(animateID,startpos,endpos,movetype,length,wait,acceleration,relativemove,command,runcommandat,ID)
    ...
    def movemenu(self,moveto,slide='none',length=30,backchainadd=True):
##        print('moving from:',self.activemenu,'-',moveto,self.queuedmenumove)
        if self.queuedmenumove[0]<0 or slide=='none':
            if self.activemenu in self.windowedmenunames and moveto == self.activemenu:
                self.menuback(slide,length)
            else:
                if backchainadd:
    ...
    def menuback(self,slide='none',length=30):
##        print('moving from:',self.activemenu,'-',self.backchain[-1],self.queuedmenumove)
        if self.queuedmenumove[0]<0 or slide=='none':
            if len(self.backchain)>0:
                if slide=='none':
                    self.activemenu = self.backchain[-1]
                else:
                    self.slidemenu(self.activemenu,self.backchain[-1],slide,length) 
    ...
    def slidemenu(self,menufrom,menuto,slide,length):
        ...
        self.makeanimation(menufrom,'current',dirr,'sinout',length,command=lambda: self.slidemenuin(menuto,length,dirr),runcommandat=length,queued=True,menu=True,relativemove=True)
        self.makeanimation(menufrom,'current',[dirr[0]*-1,dirr[1]*-1],'linear',1,menu=True,relativemove=True)
    def slidemenuin(self,moveto,length,dirr):
...
class TEXT:
    def __init__(self,x,y,text,size,menu,menuexceptions,col,center,font,bold,maxwidth,backingborder,backingcol,backingdraw,backingwidth,backingheight,img,colorkey,roundedcorners,antialiasing,layer,ID,ui):
        self.x = x
        self.y = y
        self.text = text
        self.size = size
...
class ANIMATION:
    def __init__(self,animateID,startpos,endpos,movetype,length,wait,acceleration,relativemove,command,runcommandat,ID):
    ...
    def finish(self,ui):
        ...
        if self.relativemove and self.wait>0 and self.endpos!='current':
            if self.startpos == 'current':
                self.startpos = (ui.IDs[self.animateID].x,ui.IDs[self.animateID].y)
            self.endpos = (self.startpos[0]+self.endpos[0],self.startpos[1]+self.endpos[1])
        ui.IDs[self.animateID].x = self.endpos[0]
        ui.IDs[self.animateID].y = self.endpos[1]
