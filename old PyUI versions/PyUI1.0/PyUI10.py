...
class UI:
    ...
    def makescroller(self,x,y,height,width=15,minh=0,maxh=400,pageh=100,starth=0,menu='main',col=(170,170,170),scrollercol=-1,scrollerwidth=11,layer=1,ID='default',returnobj=False):
        if scrollercol==-1:
            scrollercol = [max([0,a-30]) for a in col]
        if ID == 'default':
            ID = 'scroller'
        obj = SCROLLER(x,y,menu,width,height,col,scrollercol,scrollerwidth,minh,maxh,pageh,starth,layer,ID,self)
        if returnobj:
            return obj
        else:
            self.texts.append(obj)
...
class SCROLLER:
    def __init__(self,x,y,menu,width,height,col,scrollercol,scrollerwidth,minh,maxh,pageh,starth,layer,ID,ui):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    def draw(self,screen,scale):
        ...
        pygame.draw.rect(screen,self.scrollercol,rectscaler(self.sliderrect,scale))
        
    def refresh(self,ui):
        self.scrollerheight = (self.pageh/(self.maxh-self.minh-self.pageh))*self.height
        self.rect = pygame.Rect(self.x,self.y,self.width,self.height)
        self.sliderrect = pygame.Rect(self.x+self.border,self.y+self.border,self.scrollerwidth,self.scrollerheight)
