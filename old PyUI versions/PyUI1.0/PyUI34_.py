        ...
        for a in range(len(self.labeleddata)):
            ...
            for i,b in enumerate(self.labeleddata[a]):
                if b[0] == 'text':
                    ...
                    obj = ui.maketext(0,0,b[1],self.textsize,self.menu,'tabletext'+self.ID+str(a)+str(i),self.layer+0.01,self.roundedcorners,self.menuexceptions,maxwidth=self.maxwidth,scalesize=self.scalesize)
        ...
        if self.tableimages!=0:
            for a in range(len(self.tableimages)):
                for i,b in enumerate(self.tableimages[a]):
                    ...
                    elif b[0] == 'button' or b[0] == 'textbox' or b[0] == 'textobj':
                        print('reset',b[1].ID)
                        b[1].anchor = [self.x,self.y]
                        b[1].objanchor = [0,0]
                        b[1].startx = (self.x+self.linesize*(i+1)+self.boxwidthsinc[i])-self.anchor[0]
                        b[1].starty = (self.y+self.linesize*(a+1)+self.boxheightsinc[a])-self.anchor[1]
                        b[1].x = (self.x+(self.linesize*(i+1)+self.boxwidthsinc[i])/self.dirscale[0])
                        b[1].y = (self.y+(self.linesize*(a+1)+self.boxheightsinc[a])/self.dirscale[1])
                ...
                for b in [self.tableimages[c][a] for c in range(len(self.tableimages))]:
                    if b[0] == 'text':
                        if minn<b[1].get_width():
                            minn = b[1].get_width()+self.horizontalspacing*2*ui.scale
                    elif b[0] == 'button' or b[0] == 'textobj':
                        if minn<b[1].textimage.get_width():
                            minn = b[1].textimage.get_width()+b[1].horizontalspacing*2*ui.scale
                ...
                for b in [self.tableimages[a][c] for c in range(len(self.tableimages[0]))]:
                    if b[0] == 'text':
                        if minn<b[1].get_height():
                            minn = b[1].get_height()+self.verticalspacing*2*self.scale
                    elif b[0] == 'button' or b[0] == 'textobj':
                        print(b[1].textimage.get_height(),b[1].verticalspacing)
