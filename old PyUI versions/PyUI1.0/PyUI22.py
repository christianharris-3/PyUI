...
class UI:
    ...
    def makeanimation(self,animateID,startpos,endpos,movetype='linear',length=30,acceleration=1,command=emptyfunction,runcommandat=-1,queued=True,menu=False,relativemove=False,ID='default'):
        if menu:
            ...
            if endpos == 'current':
                endpos = (self.IDs[animateID].x,self.IDs[animateID].y)
            if ID == 'default':
...
class ANIMATION:
    ...
    def gencordlist(self):
        ...
        for a in range(self.length):
            self.cordlist.append((self.startpos[0]+(self.endpos[0]-self.startpos[0])*(sum(self.speedlist[:a])),self.startpos[1]+(self.endpos[1]-self.startpos[1])*(sum(self.speedlist[:a]))))
    def animate(self,ui):
        self.wait-=1
        if self.wait == 0:
