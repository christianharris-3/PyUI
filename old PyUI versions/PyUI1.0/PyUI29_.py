...
class UI:
    ...
    def rendertextlined(self,text,size,col='default',backingcol=(150,150,150),font='default',width=-1,bold=False,antialiasing=True,center=False,spacing=0,imgin=False,img='',scale='default'):
        if font=='default': font=self.defaultfont
        if col == 'default': col = self.defaulttextcol
...
class GUI_ITEM:
    def __init__(self,ui,x,y,width,height,menu='main',ID='',layer=1,roundedcorners=0,menuexceptions=[],
                 ...
                 scalesize=True,scalex=True,scaley=True,
    ...
    def resetcords(self,ui):
        self.refreshscale(ui)
        if self.center: self.startobjanchor[0]=self.width/2
        if self.centery: self.startobjanchor[1]=self.height/2
...
##        print(self.anchor,self.objanchor)
        self.x = self.anchor[0]/self.dirscale[0]+self.startx-self.objanchor[0]/self.dirscale[0]
        self.y = self.anchor[1]/self.dirscale[1]+self.starty-self.objanchor[1]/self.dirscale[1]
##        print(self.x,self.y)
##        print('--')
        self.refreshcords(ui)
...
class BUTTON(GUI_ITEM):
    ...
    def gentext(self,ui):
        if type(self.img) == str:
            self.textimage = ui.rendertextlined(self.text,self.textsize,font=self.font,width=self.width,bold=self.bold,center=True,imgin=True,img=self.img,scale=self.scale)
        else:
        ...
        if type(self.toggleimg) == str:
            self.toggletextimage = ui.rendertextlined(self.toggletext,self.textsize,font=self.font,width=self.width,bold=self.bold,center=True,imgin=True,img=self.toggleimg,scale=self.scale)
        else:
    ...
    def render(self,screen,ui):
        ...
        self.getclickedon(ui,pygame.Rect(self.x*self.dirscale[0],self.y*self.dirscale[1],self.width*self.scale,self.height*self.scale))
        if self.clickedon > -1:
    ...
    def draw(self,ui,screen):
        ...
        pygame.draw.rect(screen,self.backingcol,rectscaler(pygame.Rect(self.x,self.y,self.width,self.height),self),border_radius=int(self.roundedcorners*self.scale))
        pygame.draw.rect(screen,col,innerrect,border_radius=int((self.roundedcorners-self.border)*self.scale))
        if not self.toggle:
            screen.blit(self.textimage,(self.x*self.dirscale[0]+(self.width/2+self.textoffsetx)*self.scale-self.textimage.get_width()/2,self.y*self.dirscale[1]+(self.height/2+self.textoffsetx)*self.scale-self.textimage.get_height()/2))
        else:
            screen.blit(self.toggletextimage,(self.x*self.dirscale[0]+(self.width/2+self.textoffsetx)*self.scale-self.toggletextimage.get_width()/2,self.y*self.dirscale[1]+(self.height/2+self.textoffsetx)*self.scale-self.toggletextimage.get_height()/2))
