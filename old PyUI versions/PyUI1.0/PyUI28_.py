...
def rectscaler(rect,scale,offset=(0,0)):
    if type(scale) == UI:
        ui = scale
        return pygame.Rect((rect.x-offset[0])*ui.dirscale[0],(rect.y-offset[1])*ui.dirscale[1],rect.w*ui.scale,rect.h*ui.scale)
    else:
...
class UI:
    ...
    def resetscreen(self,screen):
        ...
        print('new screen size',self.screenw,self.screenh)
...
##        self.edgebind()
##    def edgebind(self,ID=0):
##        if ID == 0:
##            temp = [a.ID for a in self.items]
##        else:
##            temp = [ID]
##        print(self.dirscale)
##        for a in temp:
##            if self.IDs[a].edgebound[1]: self.IDs[a].x = (self.screenw-self.IDs[a].startx)
##            elif self.IDs[a].edgebound[3]: self.IDs[a].x = (self.IDs[a].startx)
##            if self.IDs[a].edgebound[0]: self.IDs[a].y = (self.IDs[a].starty)
##            elif self.IDs[a].edgebound[2]: self.IDs[a].y = (self.screenh-self.IDs[a].starty)
##            print(self.IDs[a].x,self.IDs[a].y)
##            if type(self.IDs[a]) in [TABLE,TEXTBOX,TEXT,SCROLLER]:
##                self.IDs[a].refreshcords(self)
    ...
    def rendertextlined(self,text,size,col='default',backingcol=(150,150,150),font='default',width=-1,bold=False,antialiasing=True,center=False,spacing=0,imgin=False,img=''):
        if font=='default': font=self.defaultfont
        if col == 'default': col = self.defaulttextcol
        if width==-1 and center: center = False
        textsize*=self.scale
        if width!=-1: width*=self.scale
        imgchr = '@'
        imgnames = []
        imgwidthid = 0
        if imgin:
        ...
        while len(lines)>0:
            ...
            if width!=-1:
                ...
                while chrwidth>width:
                    ...
                    if textgen.size(lines[0])[0] != imgchr :
                        ...
                        chrwidth = self.rendershape(imgnames[imgwidthid],size,col).get_width()
                        imgwidthid-=1
            if imgin: lines[0] = lines[0].replace(imgchr,'{'+imgnames[imgwidthid]+'}')
            ...
            imgwidthid+=len(lines[0].split(imgchr))-1
            del lines[0]
            if newline!='':
...
##        self.edgebind(ID)
    ...
    def makebutton(self,x,y,text,textsize=50,command=emptyfunction,menu='main',ID='',layer=1,roundedcorners=0,menuexceptions=[],width=-1,height=-1,
                 anchor=(0,0),center=False,centery=-1,img='none',font='default',bold=False,antialiasing=True,pregenerated=True,
                 border=3,upperborder=-1,lowerborder=-1,rightborder=-1,leftborder=-1,
                 runcommandat=0,col=-1,textcol=-1,backingcol=-1,hovercol=-1,clickdownsize=4,clicktype=0,textoffsetx=0,textoffsety=0,
                 dragable=False,colorkey=(255,255,255),toggle=False,toggleable=False,toggletext=-1,toggleimg='none',togglecol=-1,togglehovercol=-1,bindtoggle=[],verticalspacing=0,horizontalspacing=8,
                 backingdraw=False,borderdraw=False,returnobj=False):
        obj = BUTTON(self,x,y,width,height,menu,ID,layer,roundedcorners,menuexceptions,
                 anchor,center,centery,text,textsize,img,font,bold,antialiasing,pregenerated,
                 ...
                 dragable,colorkey,toggle,toggleable,toggletext,toggleimg,togglecol,togglehovercol,bindtoggle,verticalspacing,horizontalspacing,backingdraw=backingdraw,borderdraw=borderdraw)
        obj.autoscale(self,width,height)
    ...
    def maketextbox(self,x,y,title,width,lines=1,menu='main',height=-1,border=4,menuexceptions=[],edgebound=(1,0,0,1),titlesize=50,titleheight=-1,titlecenter=True,textsize=40,textcenter=False,titlefont='default',titlebold=False,col='default',backingcol=-1,titlecol='default',textcol='default',upperborder=0,lowerborder=0,rightborder=0,leftborder=0,center=True,chrlimit=1000,numsonly=False,enterreturns=False,command=emptyfunction,commandifenter=True,commandifkey=False,selectcol=-1,selectbordersize=2,selectshrinksize=0,roundedcorners=0,clicktype=0,textoffsetx=0,textoffsety=0,font='default',bold=False,cursorsize=-1,layer=1,ID='default',returnobj=False):
        if font=='default': font=self.defaultfont
        if textcol == 'default': textcol = self.defaulttextcol
        ...
        if titleheight == -1:
            if title == '':
                ...
                titleheight = titleimage.get_height()
        heightgetter = self.rendertext('Tg',textsize,textcol,font,bold)
        if height == -1:
            height = titleheight+border*2+heightgetter.get_height()*lines
        if not center:
            x+=width/2
            y+=titleheight/2
        if cursorsize == -1:
            cursorsize = self.gettextsize('Tg',font,textsize,bold)[1]-2
        if ID == 'default':
            ID = 'textbox '+title
        obj = TEXTBOX(x,y,width,height,border,titleheight,roundedcorners,title,titlecenter,titlesize,titlecol,titlefont,titlebold,backingcol,textcenter,textsize,textcol,col,upperborder,lowerborder,rightborder,leftborder,chrlimit,numsonly,enterreturns,command,commandifenter,commandifkey,selectcol,selectbordersize,selectshrinksize,cursorsize,menu,menuexceptions,edgebound,clicktype,titleimage,textoffsetx,textoffsety,font,bold,layer,ID,self)
    ...
    def maketext(self,x,y,text,size,menu='main',menuexceptions=[],edgebound=(1,0,0,1),col='default',center=True,font='default',bold=False,maxwidth=-1,border=4,backingcol='default',backingdraw=0,backingwidth=-1,backingheight=-1,img='none',colorkey=(255,255,255),roundedcorners=0,layer=1,ID='default',antialiasing=True,pregenerated=True,returnobj=False):
        if font == 'default': font = self.defaultfont
        if col == 'default': col = self.defaulttextcol
        if backingcol == 'default': backingcol = self.defaultbackingcol
        if ID == 'default':
            ID = 'text '+text
        obj = TEXT(x,y,text,size,menu,menuexceptions,edgebound,col,center,font,bold,maxwidth,border,backingcol,backingdraw,backingwidth,backingheight,img,colorkey,roundedcorners,antialiasing,pregenerated,layer,ID,self)
        if returnobj:
            return obj
        else:
...
class GUI_ITEM:
    def __init__(self,ui,x,y,width,height,menu='main',ID='',layer=1,roundedcorners=0,menuexceptions=[],
                 anchor=(0,0),center=False,centery=-1,text='',textsize=50,img='none',font='default',bold=False,antialiasing=True,pregenerated=True,
                 border=3,upperborder=-1,lowerborder=-1,rightborder=-1,leftborder=-1,
                 ...
                 dragable=False,colorkey=(255,255,255),toggle=False,toggleable=False,toggletext=-1,toggleimg='none',togglecol=-1,togglehovercol=-1,bindtoggle=[],verticalspacing=0,horizontalspacing=8,
                 selectcol=-1,selectbordersize=2,selectshrinksize=0,cursorsize=1,textcenter=False,chrlimit=10000,numsonly=False,enterreturns=False,commandifenter=True,commandifkey=False,
                 data='empty',titles=[],boxwidth=-1,boxheight=-1,linesize=2,datatable=[],
                 backingdraw=False,borderdraw=False,scrollercol=-1,scrollerwidth=-1,pageheight=15,
        ...
        if col == -1: col = ui.defaultcol
        self.col = col
        if textcol == -1: textcol = ui.defaulttextcol
        self.textcol = textcol
        if backingcol == -1: backingcol = shiftcolor(col,20)
        self.backingcol = backingcol
        self.bordercol = backingcol
        if hovercol == -1: hovercol = shiftcolor(col,-20)
        self.hovercol = hovercol
        if togglecol == -1: togglecol = shiftcolor(col,-50)
        self.togglecol = togglecol
        if togglehovercol == -1: togglehovercol = shiftcolor(togglecol,-20)
        self.togglehovercol = togglehovercol
        if selectcol == -1: selectcol = shiftcolor(col,20)
        self.selectcol = selectcol
        if scrollercol == -1: scrollercol = shiftcolor(col,-30)
        self.scrollercol = scrollercol
        if slidercol == -1: slidercol = shiftcolor(col,-30)
        self.slidercol = slidercol
        if sliderbordercol == -1: sliderbordercol = shiftcolor(col,-10)
    ...
    def gentext(self,ui):
        if type(self.img) == str:
            self.textimage = ui.rendertextlined(self.text,self.textsize,self.textcol,self.col,self.font,self.width,self.bold,True,True,True,True,self.img)
        else:
    ...
    def resetcords(self,ui):
        self.tempanchor = self.anchor[:]
        if type(self.tempanchor[0]) == str:
            exec('val=',self.tempanchor[0].replace('w',str(ui.screenw)))
            self.tempanchor[0] = val
        if type(self.tempanchor[1]) == str:
            exec('val=',self.tempanchor[1].replace('h',str(ui.screenw)))
            self.tempanchor[1] = val

        self.x = self.startx+self.tempanchor[0]
        self.y = self.starty+self.tempanchor[1]

    def refreshcords(self,ui):
        self.colliderect = pygame.Rect(self.x,self.y,self.width,self.height)
        self.drawx = (self.tempanchor[0]+(self.x-self.tempanchor[0]))*ui.dirscale[0]
        self.drawy = (self.tempanchor[1]+(self.y-self.tempanchor[1]))*ui.dirscale[1]
        self.child_refreshcords(ui)
        
    def toggleflip(self,_):
        pass
    def getclickedon(self,ui,rect='default',runcom=True,drag=True):
        if rect == 'default':
            rect = self.colliderect
        self.clickedon = -1
        self.hovering = False
        if rect.collidepoint(ui.mpos):
            if ui.mprs[self.clicktype] and (ui.mouseheld[self.clicktype][1]>0 or self.holding):
                if ui.mouseheld[self.clicktype][1] == ui.buttondowntimer:
                    self.clickedon = 0
                    self.holding = True
                    self.holdingcords = [ui.mpos[0]-self.x,ui.mpos[1]-self.y]
                    if self.toggleable:
        ...
        if ui.mprs[self.clicktype] and self.holding:
            ...
            if self.dragable and drag:
                self.x = ui.mpos[0]-self.holdingcords[0]
                self.y = ui.mpos[1]-self.holdingcords[1]
            if self.runcommandat == 1 and runcom:
                self.command()
        elif not ui.mprs[self.clicktype]:
            if self.holding and self.runcommandat>0 and runcom:
                self.clickedon = 2
                if pygame.Rect(self.x-self.width/2,self.y-self.height/2,self.width,self.height).collidepoint(ui.mpos):
                    self.command()
            self.holding = False
        return False
...
class BUTTON(GUI_ITEM):
    ...
    def gentext(self,ui):
        if type(self.img) == str:
            self.textimage = self.rendertextlined(self.text,self.textsize,font=self.font,width=self.width,bold=self.bold,center=True,imgin=True,img=self.img)
        else:
        ...
        if type(img) == str:
            self.toggletextimage = self.rendertextlined(self.toggletext,self.textsize,font=self.font,width=self.width,bold=self.bold,center=True,imgin=True,img=self.toggleimg)
        else:
            self.toggletextimage = self.img
            self.toggletextimage = pygame.transform.scale(self.img,(self.textsize,img.get_width()*self.textsize/img.get_height()))
    def autoscale(self,ui,width,height):
        if width == -1:
            self.width = max([textimage.get_width(),toggletextimage.get_width()])/ui.scale+self.horizontalspacing*2+self.leftborder+self.rightborder
        if height == -1:
            self.height = max([textimage.get_height(),toggletextimage.get_height()])/ui.scale+self.verticalspacing*2+self.upperborder+self.lowerborder
##    def gentext(self,ui):
##        if type(self.img) == str:
##            self.basetextimage = ui.rendertextlined(self.text,self.textsize*ui.scale,self.textcol,self.col,self.font,self.width*ui.scale,self.bold,True,True,True,True,self.img)
##        else:
##            scale = self.textsize/self.img.get_height()*ui.scale
##            self.basetextimage = pygame.transform.scale(self.img,(self.img.get_width()*scale,self.img.get_height()*scale))
##            self.basetextimage.set_colorkey(self.colorkey)
##        if type(self.img) == str:
##            self.basetextimage = ui.rendertextlined(self.text,self.textsize*ui.scale,self.textcol,self.col,self.font,self.width*ui.scale,self.bold,True,True,True,True,self.img)
##        else:
##            scale = self.textsize/self.img.get_height()*ui.scale
...
##            self.basetextimage.set_colorkey(self.colorkey)
    ...
    def child_refreshcords(self,ui):
        self.innerwidth = self.width-self.leftborder-self.rightborder
        self.innerheight = self.height-self.upperborder-self.lowerborder
    ...
    def render(self,screen,ui):
        ...
        if self.clickedon > -1:
            for a in self.bindtoggle:
                if a!=self.ID:
                    ui.IDs[a].toggle = Fales
        self.draw(ui,screen)
##        if self.clickedon == 0:
##            if self.toggle: self.draw(screen,ui,self.hovercol,pygame.Rect(self.x-self.innerwidth/2+self.clickdownsize,self.y-self.innerheight/2+self.clickdownsize,self.innerwidth-self.clickdownsize*2,self.innerheight-+self.clickdownsize*2))
...
##                        return True
##            else:
##                if self.toggle: self.draw(screen,ui,self.hovercol)
##                else: self.draw(screen,ui,self.togglehovercol)
##        else:
##            if self.toggle: self.draw(screen,ui,self.col)
##            else: self.draw(screen,ui,self.togglecol)
##        if ui.mprs[self.clicktype] and self.holding:
##            if self.dragable:
##                self.x = ui.mpos[0]-self.holdingcords[0]
##                self.y = ui.mpos[1]-self.holdingcords[1]
##            if self.runcommandat == 1:
##                return True
##        elif not ui.mprs[self.clicktype]:
##            if self.holding and self.runcommandat>0:
##                self.holding = False
##                if pygame.Rect(self.x-self.width/2,self.y-self.height/2,self.width,self.height).collidepoint(ui.mpos): return True
##            self.holding = False
##        return False
    def toggleflip(self,ui):
        pass
    def draw(self,ui,screen):
        col = self.col
        innerrect = pygame.Rect(self.x+self.leftborder,self.y,self.innerwidth,self.innerheight)
        if self.holding or self.hovering: col = self.hovercol
        if self.holding: innerrect = pygame.Rect(self.x-self.innerwidth/2,self.y-self.innerheight/2,self.innerwidth,self.innerheight)
        
        pygame.draw.rect(screen,self.bordercol,rectscaler(pygame.Rect(self.drawx,self.y-self.height/2,self.width,self.height),ui),border_radius=int(self.roundedcorners*ui.scale))
        pygame.draw.rect(screen,col,rectscaler(innerrect,ui),border_radius=int((self.roundedcorners-self.border)*ui.scale))
        if self.toggle:
            screen.blit(self.textimage,((self.x+self.textoffsetx)*ui.dirscale[0]-self.textimage.get_width()/2,(self.y+self.textoffsety)*ui.dirscale[1]-self.textimage.get_height()/2))
...
class TEXTBOX:
    ...
    def gentitle(self,ui):
        self.titleimage = ui.rendertext(self.title,self.titlesize*ui.scale,self.titlecol,self.titlefont,self.titlebold,backingcol=self.backingcol,imgin=True)
        self.titleimagerect = self.titleimage.get_rect()
        if self.titlecenter: self.titleimagerect.center = (self.x*ui.scale,self.y*ui.scale)
        else:
    ...
    def gentext(self,ui):
        ...
        for a in self.chrcorddatalined:
            self.chrcorddata+=a
        self.textimage = ui.rendertextlined(self.text,self.textsize*ui.scale,self.textcol,self.backingcol,self.font,(self.width-self.border*2-self.leftborder-self.rightborder-self.scrolleron*self.scroller.width)*ui.scale,self.bold,center=self.textcenter)
        self.textimagerect = self.textimage.get_rect()
        self.textimagerect.width/=ui.scale
        self.textimagerect.height/=ui.scale
        if self.textcenter: self.textimagerect.center = (self.x+self.textoffsetx+(self.leftborder+self.rightborder)/2-self.scrolleron*self.scroller.width/2,self.y+self.titleheight/2+self.border+self.textimagerect.height/2+self.textoffsety+self.upperborder)
        else:
    ...
    def refreshcursor(self):
        ...
        if self.typingcursor != -1: self.linecenter = [self.chrcorddata[self.typingcursor][1][0]+self.chrcorddata[self.typingcursor][2][0]/2+imageoffset,self.chrcorddata[self.typingcursor][1][1]]
            ...
            self.linecenter = [imageoffset,self.textsize*0.3]
        inc = 0      
    ...
    def refreshcords(self,ui):
        ...
        if self.titlecenter: self.titleimagerect.center = (self.x*ui.scale,self.y*ui.scale)
            ...
            self.titleimage.x = (self.x-self.width/2)*ui.scale
            self.titleimage.y = (self.y-self.titleheight/2)*ui.scale
    ...
    def render(self,screen,ui):
        ...
        if self.innerrect.collidepoint(ui.mpos) and ui.mprs[self.clicktype]:
            ...
            if ui.mprs[self.clicktype] and ui.mouseheld[self.clicktype][1] == ui.buttondowntimer:
                self.clickstartedinbound = False
                self.selected = False
            if not self.rect.collidepoint(mpos) and ui.mprs[self.clicktype] and not ui.mouseheld[self.clicktype]:
                self.selected = False
                self.textselected = [False,0,0]
        ...
        if ui.mprs[self.clicktype] and ui.mouseheld[self.clicktype][1] != ui.buttondowntimer and self.clickstartedinbound:
            ...
            if self.scrolleron:
                if ui.mpos[1]<self.y+self.titleheight/2+self.border:
                    self.scroller.scroll+=(ui.mpos[1]-(self.y+self.titleheight/2+self.border))/10
                    self.scroller.limitpos()
                elif ui.mpos[1]>self.y-self.titleheight/2+self.height:
                    self.scroller.scroll+=(ui.mpos[1]-(self.y-self.titleheight/2+self.height))/10
                    self.scroller.limitpos()
    ...
    def findclickloc(self,mpos=-1,relativempos=-1):
        if len(self.chrcorddata)==0:
            ...
            if self.relativempos[0]<self.chrcorddatalined[dis[0]][hdis[0]][1][0]:
                strpos-=1
            return strpos
    def draw(self,screen,ui,clicking):
        pygame.draw.rect(screen,self.backingcol,rectscaler(self.rect,ui),border_radius=int(self.roundedcorners*ui.scale))
        pygame.draw.rect(screen,self.textbackingcol,rectscaler(self.innerrect,ui),border_radius=int(self.roundedcorners*ui.scale))
        if self.selected: pygame.draw.rect(screen,self.selectcol,rectscaler(self.selectrect,ui),self.selectbordersize,border_radius=int((self.roundedcorners+self.selectbordersize)*ui.scale))
        screen.blit(self.titleimage,self.titleimagerect)

        surf = pygame.Surface(((self.width-self.border*2-self.scrolleron*self.scroller.width)*ui.scale,(self.height-self.titleheight-self.border*2)*ui.scale))
...
##        print(rectscaler(self.textimagerect,scale,offset),self.textimagerect,scale,offset)
        surf.blit(self.textimage,rectscaler(self.textimagerect,ui,offset))
        if self.typeline>20 and self.selected:
            pygame.draw.line(surf,self.textcol,((self.linecenter[0]+self.leftborder)*ui.scale,(self.linecenter[1]-self.cursorsize/2+self.upperborder-self.scroller.scroll)*ui.scale),((self.linecenter[0]+self.leftborder)*ui.scale,(self.linecenter[1]+self.cursorsize/2+self.upperborder-self.scroller.scroll)*ui.scale),2)
        if self.textselected[0] and self.textselected[1]!=self.textselected[2]:
            ...
            for a in range(min([self.textselected[1],self.textselected[2]]),max([self.textselected[1],self.textselected[2]])):
                if self.chrcorddata[a][0] != '\n':
                    trect[0] = (self.leftborder+self.chrcorddata[a][1][0]-self.chrcorddata[a][2][0]/2)*ui.scale
                    trect[1] = (self.upperborder+self.chrcorddata[a][1][1]-self.chrcorddata[a][2][1]/2-self.scroller.scroll)*ui.scale
        ...
        screen.blit(surf,(self.x*ui.dirscale[0]-(self.width/2+self.border)*scale,self.y*ui.dirscale[1]+(self.border+self.titleheight/2)*ui.scale))
...
class TABLE:
    def __init__(self,x,y,rows,colomns,data,titles,boxwidths,boxheight,spacing,menu,menuexceptions,edgebound,boxcol,boxtextcol,boxtextsize,boxcenter,boxfont,boxbold,titlefont,titlebold,titleboxcol,titletextcol,titletextsize,titlecenter,linesize,linecol,roundedcorners,layer,ID,ui):
        self.x = x
        self.y = y
        self.startx = x
        self.starty = y
    ...
    def gettablewidths(self,ui):
        ...
        self.width = self.boxwidthtotal+self.linesize*(self.colomns+1)
    ...
    def gettableheights(self,ui):
        ...
        self.height = self.boxheighttotal+self.linesize*(self.rows+2)
    ...
    def gentext(self,ui):
        ...
        for a in range(len(self.labeleddata)):
            ...
            for i,b in enumerate(self.labeleddata[a]):
                if b[0] == 'text':
                    if self.boxwidth == -1:
                        if a==0: img = ui.rendertext(b[1],self.titletextsize,self.titletextcol,self.titlefont,self.titlebold,backingcol=self.boxcol)
                        else: img = ui.rendertext(b[1],self.boxtextsize,self.boxtextcol,self.boxfont,self.boxbold,backingcol=self.boxcol)
                    else:
                        if type(self.boxwidth) == int:
                            if a==0: img = ui.rendertextlined(b[1],self.titletextsize*ui.scale,self.titletextcol,self.titleboxcol,self.titlefont,self.boxwidth*ui.scale,self.titlebold,center=self.titlecenter)
                            else: img = ui.rendertextlined(b[1],self.boxtextsize*ui.scale,self.boxtextcol,self.boxcol,self.boxfont,self.boxwidth*ui.scale,self.boxbold,center=self.boxcenter)
                        else:
                            if a==0: img = ui.rendertextlined(b[1],self.titletextsize*ui.scale,self.titletextcol,self.titleboxcol,self.titlefont,self.boxwidth[i]*ui.scale,self.titlebold,center=self.titlecenter)
                            else: img = ui.rendertextlined(b[1],self.boxtextsize*ui.scale,self.boxtextcol,self.boxcol,self.boxfont,self.boxwidth[i]*ui.scale,self.boxbold,center=self.boxcenter)
                    rec = img.get_rect()
...
class TEXT:
    def __init__(self,x,y,text,size,menu,menuexceptions,edgebound,col,center,font,bold,maxwidth,backingborder,backingcol,backingdraw,backingwidth,backingheight,img,colorkey,roundedcorners,antialiasing,pregenerated,layer,ID,ui):
        self.x = x
        self.y = y
        self.startx = x
        self.starty = y
    ...
    def draw(self,ui,screen):
        if self.backingdraw:
            pygame.draw.rect(screen,self.backingcol,rectscaler(self.backingrect,ui.scale),border_radius=int(self.roundedcorners*ui.scale))
        if self.pregenerated:
            screen.blit(self.textimage,self.textrect)
        else:
            ui.write(screen,self.x,self.y,self.text,self.size,self.col,self.center,self.font,self.bold,self.antialiasing)
    ...
    def gentext(self,ui):
        if type(self.img) == str:
            if self.maxwidth == -1:
                self.textimage = ui.rendertext(self.text,self.size*ui.scale,self.col,self.font,self.bold,self.antialiasing,backingcol=self.backingcol,imgin=True,img=self.img)
            else:
                self.textimage = ui.rendertextlined(self.text,self.size*ui.scale,self.col,self.backingcol,self.font,self.maxwidth*ui.scale,self.bold,self.antialiasing,self.center,imgin=True,img=self.img)
        else:
    ...
    def refreshcords(self,ui):
        ...
        if self.center:
            self.textrect.center = (self.x*ui.dirscale[0],self.y*ui.dirscale[1]+self.textimage.get_height()/2)
        else:
            self.textrect.x = self.x*ui.dirscale[0]
            self.textrect.y = self.y*ui.dirscale[1]
