...
class UI:
    ...
    def rendergui(self,screen):
        ...
        #print(self.mprs,self.mouseheld,self.buttondowntimer)
    ...
    def rendershape(self,name,size,col='default'):
        ...
        if name[:4] == 'tick':
            ...
            surf = pygame.Surface((size*(3**0.5)/2,size))
            surf.fill(backcol)
            vals = self.getshapedata(name,['rounded'],[0.0])
            rounded = vals[0]
            sin = math.sin(math.pi/3)
            cos = math.cos(math.pi/3)
            points = [(size*rounded*cos,size*rounded*sin),(size*(1-rounded)*(3**0.5)/2,size*0.5),(size*rounded*cos,size-size*(rounded)*sin)]
    ...
    def makebutton(self,x,y,text,textsize,command=emptyfunction,menu='main',col='default',bordercol=-1,hovercol=-1,textcol='default',img='none',colorkey=(255,255,255),width=-1,height=-1,border=3,clickdownsize=3,verticalspacing=0,horizontalspacing=8,roundedcorners=0,clicktype=0,textoffsetx=0,textoffsety=0,toggle=True,togglecol=-1,togglehovercol=-1,toggleable=False,drawifoff=True,bindtoggle=[],font='default',bold=False,layer=1,ID='default',returnobj=False):
        ...
        if ID == 'default':
            ID = 'button '+text
        obj = BUTTON(x,y,text,textsize,command,menu,col,bordercol,hovercol,textcol,img,colorkey,width,height,border,clickdownsize,roundedcorners,clicktype,textoffsetx,textoffsety,toggle,togglecol,togglehovercol,toggleable,bindtoggle,drawifoff,font,bold,layer,ID,self)
        self.addid(ID,obj)
        if returnobj:
            return obj
        else:
    ...
    def makecheckbox(self,x,y,textsize=80,command=emptyfunction,menu='main',text='',col=(255,255,255),bordercol='default',hovercol=-1,textcol='default',img='tick',colorkey=(255,255,255),width=-1,height=-1,border=4,clickdownsize=1,verticalspacing=-15,horizontalspacing=-15,roundedcorners=0,clicktype=0,textoffsetx=0,textoffsety=0,toggle=True,togglecol=-1,togglehovercol=-1,toggleable=True,drawifoff=False,bindtoggle=[],font='default',bold=False,layer=1,ID='tickbox',returnobj=False):
        ...
        self.makebutton(x,y,text,textsize,command,menu,col,bordercol,hovercol,textcol,img,colorkey,width,height,border,clickdownsize,verticalspacing,horizontalspacing,roundedcorners,clicktype,textoffsetx,textoffsety,toggle,togglecol,togglehovercol,toggleable,drawifoff,bindtoggle,font,bold,layer,ID,returnobj)
    ...
    def makescroller(self,x,y,height,command=emptyfunction,width=15,minh=0,maxh=-1,pageh=100,starth=0,menu='main',col='default',scrollercol=-1,hovercol=-1,clickcol=-1,scrollerwidth=11,clicktype=0,layer=1,ID='default',returnobj=False):
        if col == 'default':
        ...
        if maxh == -1:
            maxh = height
        obj = SCROLLER(x,y,menu,command,width,height,col,scrollercol,hovercol,clickcol,scrollerwidth,minh,maxh,pageh,starth,clicktype,layer,ID,self)
        if returnobj:
            return obj
        else:
            self.scrollers.append(obj)
            self.addid(ID,obj)
    def makeslider(self,x,y,width,height,maxp=100,menu='main',command=emptyfunction,col='default',slidercol=-1,sliderbordercol=-1,hovercol=-1,clickcol=-1,clickdownsize=2,bordercol=-1,border=2,slidersize=-1,img='none',colorkey=(255,255,255),minp=0,startp=0,style='square',roundedcorners=0,barroundedcorners=-1,dragable=True,clicktype=0,layer=1,ID='default',returnobj=False):
        if col == 'default':
            col = self.defaultcol
        if bordercol==-1:
            bordercol = [min([255,a+20]) for a in col]
        ...
        obj = SLIDER(x,y,width,height,menu,command,col,bordercol,slidercol,sliderbordercol,hovercol,clickdownsize,border,slidersize,roundedcorners,barroundedcorners,img,colorkey,minp,maxp,startp,dragable,clicktype,layer,ID,self)
        if returnobj:
            return obj
        else:
...
class BUTTON:
    def __init__(self,x,y,text,textsize,command,menu,col,bordercol,hovercol,textcol,img,colorkey,width,height,border,clickdownsize,roundedcorners,clicktype,textoffsetx,textoffsety,toggle,togglecol,togglehovercol,toggleable,bindtoggle,drawifoff,font,bold,layer,ID,ui):
        self.x = x
        self.y = y
        self.text = text
...
class TEXTBOX:
    ...
    def resetscroller(self,ui):
        ...
        self.scroller = ui.makescroller(self.x+self.width/2-15-self.border/2,self.y+self.titleheight/2+self.border,self.height-self.titleheight-self.border*2,emptyfunction,15,0,self.height-self.titleheight-self.border,self.height-self.titleheight,returnobj=True)
...
class SCROLLER:
    def __init__(self,x,y,menu,command,width,height,col,scrollercol,hovercol,clickcol,scrollerwidth,minh,maxh,pageh,starth,clicktype,layer,ID,ui):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
class SLIDER:
    def __init__(self,x,y,width,height,menu,command,col,bordercol,slidercol,sliderbordercol,hovercol,clickdownsize,border,slidersize,roundedcorners,barroundedcorners,img,colorkey,minp,maxp,startp,dragable,clicktype,layer,ID,ui):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
