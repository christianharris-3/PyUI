...
class UI:
    ...
    def rendergui(self,screen):
        ...
        if self.activemenu in self.windowedmenunames:
            ...
            windowsurf = pygame.Surface((window.width,window.height))
            windowsurf.fill(window.colorkey)
            pygame.draw.rect(windowsurf,window.col,pygame.Rect(0,0,window.width,window.height),border_radius=int(window.roundedcorners))
            ...
            screen.blit(windowsurf,(window.x,window.y))
    ...
    def animate(self):
        ...
        for a in delete:
            self.delete(a)
    def makeanimation(self,animateID,startpos,endpos,movetype='linear',length=30,command=emptyfunction,runcommandat=-1,queued=True,menu=False,relativemove=False,skiptoscreen=False,acceleration=1,ID='default'):
        if menu:
            for a in self.items:
