...
class UI:
    ...
    def renderguiobject(self,a,screen):
        if type(a) == BUTTON:
            ...
            a.render(self,screen,self.scale)
    ...
    def rendershapebezier(self,name,size,col,backcol,failmessage):
        ...
        for b in splines:
            points = []
            for a in b:
                points+=bezierdrawer([((a[c][0]-minus[0])*mul,(a[c][1]-minus[1])*mul) for c in range(len(a))],0,False)
            pygame.draw.polygon(surf,col,points)
    ...
    def makebutton(self,x,y,text,textsize,command=emptyfunction,menu='main',menuexceptions=[],col='default',bordercol=-1,hovercol=-1,textcol='default',img='none',colorkey=(255,255,255),runcommandat=0,width=-1,height=-1,border=3,clickdownsize=3,verticalspacing=0,horizontalspacing=8,roundedcorners=0,clicktype=0,textoffsetx=0,textoffsety=0,toggle=True,togglecol=-1,togglehovercol=-1,toggleable=False,drawifoff=True,bindtoggle=[],dragable=False,center=True,font='default',bold=False,layer=1,ID='default',returnobj=False):
        if font=='default': font=self.defaultfont
        if textcol == 'default': textcol = self.defaulttextcol
        if col == 'default': col = self.defaultcol
        if type(img) == str:
            textimage = self.rendertextlined(text,textsize,font=font,width=width,bold=bold,center=True,imgin=True,img=img)
        else:
            textimage = img
        ...
        if width == -1:
            ...
            if height == -1:
                height = textimage.get_height()+verticalspacing*2+border*2
        else:
        ...
        if ID == 'default':
            ID = 'button '+text
        obj = BUTTON(x,y,text,textsize,command,menu,menuexceptions,col,bordercol,hovercol,textcol,img,colorkey,runcommandat,width,height,border,clickdownsize,roundedcorners,clicktype,textoffsetx,textoffsety,toggle,togglecol,togglehovercol,toggleable,bindtoggle,drawifoff,dragable,font,bold,layer,ID,self)
        self.buttons.append(obj)
        self.addid(ID,obj)
        if returnobj:
            return obj

    def makecheckbox(self,x,y,textsize=80,command=emptyfunction,menu='main',menuexceptions=[],text='',col=(255,255,255),bordercol='default',hovercol=-1,textcol='default',img='tick',colorkey=(255,255,255),runcommandat=0,width=-1,height=-1,border=4,clickdownsize=1,verticalspacing=-15,horizontalspacing=-15,roundedcorners=0,clicktype=0,textoffsetx=0,textoffsety=0,toggle=True,togglecol=-1,togglehovercol=-1,toggleable=True,drawifoff=False,bindtoggle=[],dragable=False,center=True,font='default',bold=False,layer=1,ID='tickbox',returnobj=False):
        ...
        self.makebutton(x,y,text,textsize,command,menu,menuexceptions,col,bordercol,hovercol,textcol,img,colorkey,runcommandat,width,height,border,clickdownsize,verticalspacing,horizontalspacing,roundedcorners,clicktype,textoffsetx,textoffsety,toggle,togglecol,togglehovercol,toggleable,drawifoff,bindtoggle,dragable,center,font,bold,layer,ID,returnobj)
            
    def maketextbox(self,x,y,title,width,lines=1,menu='main',height=-1,border=4,menuexceptions=[],titlesize=50,titleheight=-1,titlecenter=True,textsize=40,textcenter=False,titlefont='default',titlebold=False,col='default',backingcol=-1,titlecol='default',textcol='default',upperborder=0,lowerborder=0,rightborder=0,leftborder=0,center=True,chrlimit=1000,numsonly=False,enterreturns=False,command=emptyfunction,commandifenter=True,commandifkey=False,selectcol=-1,selectbordersize=2,selectshrinksize=0,roundedcorners=0,clicktype=0,textoffsetx=0,textoffsety=0,font='default',bold=False,cursorsize=-1,layer=1,ID='default',returnobj=False):
        if font=='default': font=self.defaultfont
        if textcol == 'default': textcol = self.defaulttextcol
    ...
    def maketext(self,x,y,text,size,menu='main',menuexceptions=[],col='default',center=True,font='default',bold=False,maxwidth=-1,border=4,backingcol='default',backingdraw=0,backingwidth=-1,backingheight=-1,img='none',colorkey=(255,255,255),roundedcorners=0,layer=1,ID='default',antialiasing=True,pregenerated=True,returnobj=False):
        if font == 'default': font = self.defaultfont
        if col == 'default': col = self.defaulttextcol
        if backingcol == 'default': backingcol = self.defaultcol
        if ID == 'default':
            ID = 'text '+text
        obj = TEXT(x,y,text,size,menu,menuexceptions,col,center,font,bold,maxwidth,border,backingcol,backingdraw,backingwidth,backingheight,img,colorkey,roundedcorners,antialiasing,pregenerated,layer,ID,self)
        if returnobj:
            return obj
        else:
    ...
    def makescroller(self,x,y,height,command=emptyfunction,width=15,minh=0,maxh=-1,pageh=100,starth=0,menu='main',menuexceptions=[],col='default',scrollercol=-1,hovercol=-1,clickcol=-1,scrollerwidth=11,runcommandat=1,clicktype=0,layer=1,ID='default',returnobj=False):
        if col == 'default':
        ...
        if maxh == -1:
            maxh = height
        obj = SCROLLER(x,y,menu,menuexceptions,command,width,height,col,scrollercol,hovercol,clickcol,scrollerwidth,minh,maxh,pageh,starth,runcommandat,clicktype,layer,ID,self)
        self.scrollers.append(obj)
        self.addid(ID,obj)
        if returnobj:
            return obj

    def makeslider(self,x,y,width,height,maxp=100,menu='main',command=emptyfunction,menuexceptions=[],col='default',slidercol=-1,sliderbordercol=-1,hovercol=-1,clickcol=-1,clickdownsize=2,bordercol=-1,border=2,slidersize=-1,increment=0,img='none',colorkey=(255,255,255),minp=0,startp=0,style='square',roundedcorners=0,barroundedcorners=-1,dragable=True,runcommandat=1,clicktype=0,layer=1,ID='default',returnobj=False):
        if col == 'default':
            col = self.defaultcol
        if bordercol==-1:
            bordercol = [min([255,a+20]) for a in col]
        ...
        obj = SLIDER(x,y,width,height,menu,menuexceptions,command,col,bordercol,slidercol,sliderbordercol,hovercol,clickdownsize,border,slidersize,increment,roundedcorners,barroundedcorners,img,colorkey,minp,maxp,startp,dragable,runcommandat,clicktype,layer,ID,self)
        self.sliders.append(obj)
        self.addid(ID,obj)
        if returnobj:
            return obj

    def makewindowedmenu(self,x,y,width,height,menu,behindmenu,col='default',isolated=True,roundedcorners=0,darken=60,colourkey=(243,244,242),ID='default'):
        if col == 'default':
            col = [max([0,a-35]) for a in self.defaultcol]
        if ID == 'default':
            ID = menu
        obj = WINDOWEDMENU(menu,behindmenu,x,y,width,height,col,roundedcorners,colourkey,isolated,darken,ID)
...
class BUTTON:
    def __init__(self,x,y,text,textsize,command,menu,menuexceptions,col,bordercol,hovercol,textcol,img,colorkey,runcommandat,width,height,border,clickdownsize,roundedcorners,clicktype,textoffsetx,textoffsety,toggle,togglecol,togglehovercol,toggleable,bindtoggle,drawifoff,dragable,font,bold,layer,ID,ui):
        self.x = x
        self.y = y
        self.text = text
    ...
    def gentext(self,ui):
        if type(self.img) == str:
            self.textimage = ui.rendertextlined(self.text,self.textsize*ui.scale,self.textcol,self.col,self.font,self.width*ui.scale,self.bold,True,True,True,True,self.img)
        else:
    ...
    def render(self,screen,scale,mpos,mprs,mouseheld,buttondowntimer):
        ...
        if mprs[self.clicktype] and self.holding:
            ...
            if self.holding and self.runcommandat>0:
                self.holding = False
                if pygame.Rect(self.x-self.width/2,self.y-self.height/2,self.width,self.height).collidepoint(mpos): return True
            self.holding = False
        return False
...
class TEXTBOX:
    ...
    def gentitle(self,ui):
        self.titleimage = ui.rendertext(self.title,self.titlesize,self.titlecol,self.titlefont,self.titlebold,backingcol=self.backingcol,imgin=True)
...
class TABLE:
    def __init__(self,x,y,rows,colomns,data,titles,boxwidths,boxheight,spacing,menu,menuexceptions,boxcol,boxtextcol,boxtextsize,boxcenter,boxfont,boxbold,titlefont,titlebold,titleboxcol,titletextcol,titletextsize,titlecenter,linesize,linecol,roundedcorners,layer,ID,ui):
        self.x = x
        self.y = y
        self.rows = rows
...
class TEXT:
    def __init__(self,x,y,text,size,menu,menuexceptions,col,center,font,bold,maxwidth,backingborder,backingcol,backingdraw,backingwidth,backingheight,img,colorkey,roundedcorners,antialiasing,pregenerated,layer,ID,ui):
        self.x = x
        self.y = y
        self.text = text
        self.size = size
    ...
    def render(self,ui,screen,scale):
        self.draw(ui,screen,scale)
    def draw(self,ui,screen,scale):
        if self.backingdraw:
...
class SCROLLER:
    def __init__(self,x,y,menu,menuexceptions,command,width,height,col,scrollercol,hovercol,clickcol,scrollerwidth,minh,maxh,pageh,starth,runcommandat,clicktype,layer,ID,ui):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
class SLIDER:
    def __init__(self,x,y,width,height,menu,menuexceptions,command,col,bordercol,slidercol,sliderbordercol,hovercol,clickdownsize,border,slidersize,increment,roundedcorners,barroundedcorners,img,colorkey,minp,maxp,startp,dragable,runcommandat,clicktype,layer,ID,ui):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
class WINDOWEDMENU:
    def __init__(self,menu,behindmenu,x,y,width,height,col,roundedcorners,colorkey,isolated,darken,ID):
        self.menu = menu
    ...
    def animate(self,ui):
        ...
        if self.wait<1:
            if self.progress<self.length:
                ...
                if type(ui.IDs[self.animateID]) in [TABLE,TEXTBOX,TEXT]:
