...
##            self.windowedmenus[self.windowedmenunames.index(self.activemenu)][2]+=5
            #window = [menu,behindmenu,x,y,width,height,col,rounedcorners,colorkey]
    ...
    def makeslider(self,x,y,width,height,maxp=100,menu='main',command=emptyfunction,col='default',slidercol=-1,sliderbordercol=-1,hovercol=-1,clickcol=-1,clickdownsize=2,bordercol=-1,border=2,slidersize=-1,img='none',colorkey=(255,255,255),minp=0,startp=0,style='square',roundedcorners=0,barroundedcorners=-1,dragable=True,runcommandat=1,clicktype=0,layer=1,ID='default',returnobj=False):
        ...
        obj = SLIDER(x,y,width,height,menu,command,col,bordercol,slidercol,sliderbordercol,hovercol,clickdownsize,border,slidersize,roundedcorners,barroundedcorners,img,colorkey,minp,maxp,startp,dragable,runcommandat,clicktype,layer,ID,self)
        if returnobj:
            return obj
        else:
            self.sliders.append(obj)
            self.addid(ID,obj)
    def makewindowedmenu(self,x,y,width,height,menu,behindmenu,col='default',isolated=True,roundedcorners=0,colourkey=(243,244,242)):
        if col == 'default':
            col = [max([0,a-35]) for a in self.defaultcol]
        self.windowedmenus.append([menu,behindmenu,x,y,width,height,col,roundedcorners,colourkey,isolated])
...
class BUTTON:
    ...
    def draw(self,screen,innercol,scale):
        ...
        pygame.draw.rect(screen,innercol,rectscaler(self.innerrect,scale),border_radius=int((self.roundedcorners-self.border)*scale))
...
class SLIDER:
    def __init__(self,x,y,width,height,menu,command,col,bordercol,slidercol,sliderbordercol,hovercol,clickdownsize,border,slidersize,roundedcorners,barroundedcorners,img,colorkey,minp,maxp,startp,dragable,runcommandat,clicktype,layer,ID,ui):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
